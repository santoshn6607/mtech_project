export_model=function(){
  ...
  X2=rbind(c(372,141,10.04,95,0),c(50,123,8,100,0))
  n=nrow(X2)
  Ct=matrix(rep(attributes(X_train)$'scaled:center',each=n),nrow=n,ncol=5)
  St=matrix(rep(attributes(X_train)$'scaled:scale',each=n),nrow=n,ncol=5)
  X2=as.matrix((X2-Ct)/St)

  learn_rate = 10^-2
  weight_val = 1.050
  cat(paste0("Running, learning rate=",learn_rate," cost weight=",weight_val,"\n"))
  ws_s=ws
  ...
  model %>% 
    ...
    saveRDS(e, file="~/newtest/testout_NN.rds")
  ...
  save_model_hdf5(model, paste0(path_name,"test.h5"))
